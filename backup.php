...
$push_root = '/root/push';
$default_folder = 'ristorantemimmo1';
$base_path = $push_root . '/' . $default_folder;
$backup_dir = '/root/backups/ristorantemimmo1';

$folders = ['ristorantemimmo1', 'consulenticaniegatti'];
...
if ($action === 'create') {
    ...
    $folder_to_backup = $default_folder;
    if ($selected_folder !== '' && in_array($selected_folder, $folders, true)) {
        $folder_to_backup = $selected_folder;
    }
    $timestamp = date('Y-m-d_H-i-s');
    $backup_name = "backup_{$folder_to_backup}_{$timestamp}.tar.gz";
    $backup_path = $backup_dir . '/' . $backup_name;
    
    $command = "tar -czf " . escapeshellarg($backup_path) . " -C " . escapeshellarg($push_root) . " " . escapeshellarg($folder_to_backup) . " 2>&1";
...
<select id="folderSelect" class="backup-select">
<option value="">Tutte le cartelle</option>
<option value="ristorantemimmo1">ristorantemimmo1</option>
<option value="consulenticaniegatti">consulenticaniegatti</option>
</select>
...
function createBackup(){const select=document.getElementById('folderSelect');const folder=select?select.value:'';const confirmMsg=folder?'Vuoi creare un backup della cartella "'+folder+'"?':'Vuoi creare un backup completo adesso?';if(!confirm(confirmMsg))return;showMessage('⏳ Creazione backup in corso...','info');const url=folder?'backup.php?action=create&folder='+encodeURIComponent(folder):'backup.php?action=create';fetch(url).then(response=>response.json()).then(data=>{if(data.success){showMessage('✅ '+data.message+' ('+data.size+')','success');loadBackups()}else{showMessage('❌ '+(data.error||'Errore'),'danger')}}).catch(error=>{showMessage('❌ Errore: '+error.message,'danger')})}
